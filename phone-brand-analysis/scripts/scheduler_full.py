...
"""
手机品牌差评分析定时任务脚本（完整版）
功能：每周自动运行一次工作流，生成分析报告并自动推送到 GitHub
"""
...
def run_workflow():
    ...
    try:
        ...
        if result and isinstance(result, dict):
            ...
            report_date = result.get("report_date", current_date)

            logger.info(f"报告日期: {report_date}")
            logger.info(f"PDF 文档URL: {document_url}")

            # 检查是否成功推送到 GitHub
            if "github_url" in result and result["github_url"]:
                logger.info(f"✅ 报告已推送到 GitHub: {result['github_url']}")
            else:
                logger.warning("⚠️  GitHub 推送可能未完成，请检查日志")

            # 保存结果到文件
            result_file = PROJECT_ROOT / "reports" / f"result_{current_date}.txt"
            result_file.parent.mkdir(exist_ok=True)
            with open(result_file, 'w', encoding='utf-8') as f:
                f.write(f"生成日期: {current_date}\n")
                f.write(f"PDF 文档URL: {document_url}\n")
                f.write(f"GitHub URL: {result.get('github_url', '未生成')}\n")
